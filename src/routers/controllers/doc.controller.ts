...
export default {
    isExist: async (
        ...
        try {
            ...
            const company = await CompanyRepository.findByNameOrFail(
                companyName
            );
    ...
    getDocsByUser: async (
        ...
        try {
            ...
            const didDocsBelongToUser = didDocsBelongToCompany.filter(
                (didDoc) =>
                    didDoc.holder === publicKey || didDoc.owner === publicKey
            );
            const documents = await Promise.all(
                didDocsBelongToUser.map(async (didDoc) => {
                    const wrappedDoc =
                        await DocRepository.findByDidDocAndCompany(
                            company,
                            didDoc
                        );
    ...
    createDoc: async (
        ...
        try {
            ...
            if (!ownerDid)
                throw new AppError(ERROR_CODES.INVALID_WRAPPED_DOCUMENT);
            ...
            const didDocExist = await DidDocRepository.isExists(
                didDocDid,
                company
            );
    ...
    updateDidDoc: async (
        req: Request<
            {},
            {},
            { companyName: string; fileName: string; didDoc: Object }
        >,
        res: Response,
        next: NextFunction
    ) => {
        try {
            const { companyName, fileName, didDoc } =
            ...
            const _didDoc = await DidDocRepository.updateDidDoc(
                company,
                fileName,
                didDoc
            );
            return res.status(200).json(OPERATION_CODES.UPDATE_SUCCESS);
        } catch (error) {
            return next(handleMongoError(error));
        }
    },
