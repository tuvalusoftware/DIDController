...
describe("DOC", function () {
        ...
        it("it should return a success message states that the wrapped document is saved successfully", (done) => {
            chai.request(server)
                ...
                .end((err, res) => {
                    ...
                    expect(JSON.stringify(res.body)).equal(
                        JSON.stringify(OPERATION_CODES.CLONE_SUCCESS)
                    );
    ...
    describe("/PUT update wrapped document", () => {
        it("it should return a 'missing params' error as the required params are not provided", (done) => {
            chai.request(server)
                .put("/api/doc/update")
                .send(EMPTY_DATA)
        ...
        it("it should return a success message states that wrapped document is updated successfully", (done) => {
            chai.request(server)
                .put("/api/doc/update")
                .send(UPDATED_WRAPPED_DOC)
                .end((err, res) => {
                    res.should.have.status(200);
                    res.body.should.be.a("object");
        ...
        it("it should GET the updated wrapped document", (done) => {
            chai.request(server)
                .get("/api/doc")
                .query({
                    companyName: UPDATED_WRAPPED_DOC.companyName,
                    fileName: UPDATED_WRAPPED_DOC.fileName,
                    only: "doc",
                })
                .end((err, res) => {
                    res.should.have.status(200);
                    ...
                    res.body.should.have
                        .property("wrappedDoc")
                        .eql(UPDATED_WRAPPED_DOC.wrappedDocument);
